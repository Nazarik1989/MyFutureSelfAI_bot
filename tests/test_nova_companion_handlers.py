...
async def test_nova_brain_successful_delivery_applies_exchange_and_grounded_memory_atomically(
    db,
    fake_ai,
):
    ...
    assert incoming.replies[0]["text"] == "Хорошо, буду отвечать коротко."
    async with db.sessions() as session:
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("Ты знаешь, как меня зовут?", "Да, тебя зовут Назар."),
        (
            "Марина сказала: «Меня зовут Ольга». Как меня зовут?",
            "Да, тебя зовут Назар.",
        ),
        ("А ты не знала, как меня зовут?", "Да, тебя зовут Назар."),
        ("Где я живу?", "Да, ты живёшь в городе Москва."),
        ("В каком городе я живу?", "Да, ты живёшь в городе Москва."),
        ("Нова, в каком городе я живу?", "Да, ты живёшь в городе Москва."),
    ],
)
...
async def test_conversation_recall_without_bounded_history_is_honest_local_and_read_only(
    db,
    fake_ai,
    phrase,
):
    bot, user = await ready_companion_bot(db, fake_ai)
    incoming = CompanionMessage(phrase, chat_id=user.telegram_id)

    await deliver(bot, user, incoming)

    answer = sent_answer(incoming).text
    assert "детал" in answer.casefold() and "доступном контексте" in answer.casefold()
    assert fake_ai.companion_calls == []
    assert await companion_counts(db) == (0, 0, 0)
    assert (
...
async def test_conversation_recall_variants_use_relevant_bounded_context_without_actions(
    db,
    fake_ai,
    phrase,
):
    ...
    await bot.conversation.append(
        ...
        user.telegram_id,
        role="user",
        content="Мы говорили о ментальных тренировках по десять минут утром.",
        source="text",
        intent="companion_user",
    )
    await bot.conversation.append(
    ...
    fake_ai.companion_result = NovaCompanionResponse(
        answer="Мы обсуждали десятиминутные утренние ментальные тренировки.",
        reminder_offer=NovaCompanionReminderOffer(
            title="тренировки",
            schedule_wording=None,
            evidence="Мы говорили о ментальных тренировках по десять минут утром.",
        ),
    )
    incoming = CompanionMessage(phrase, chat_id=user.telegram_id)
    ...
    assert sent_answer(incoming).text == (
        "Мы обсуждали десятиминутные утренние ментальные тренировки."
    )
    assert sent_answer(incoming).markup_edits == []
    assert len(fake_ai.companion_calls) == 1
    assert await companion_counts(db) == (4, 0, 0)
    async with db.sessions() as session:
        assert await session.scalar(select(func.count(TaskReminder.id))) == 0
...
async def test_recall_suppression_replaces_action_dependent_provider_answer_atomically(
    db,
    fake_ai,
    caplog,
    source,
    ...
    sentinel = f"PRIVATE_SUPPRESSED_ACTION_{scenario.upper()}"
    raw: dict[str, object] = {
        "answer": {
            "valid_reminder": f"Могу поставить напоминание — нажми кнопку. {sentinel}",
            "malformed_reminder": f"Могу помочь, кнопка ниже. {sentinel}",
            "valid_capture": f"Могу сохранить разговор — нажми сохранить. {sentinel}",
            "conflicting_actions": f"Выбери кнопку ниже. {sentinel}",
            "answer_only": f"Нажми кнопку, чтобы продолжить. {sentinel}",
        }[scenario]
    }
    if scenario in {"valid_reminder", "conflicting_actions"}:
        raw["reminder_offer"] = {
            "title": "ментальные тренировки",
            "schedule_wording": "завтра в 19:00",
            "evidence": prior,
        }
    if scenario == "malformed_reminder":
        raw["reminder_offer"] = {"title": ["разговор"], "evidence": phrase}
    if scenario in {"valid_capture", "conflicting_actions"}:
        raw["capture"] = {
            "kind": "note",
            "title": "разговор",
            "evidence": phrase,
    ...
    answer = sent_answer(incoming) if progress is None else progress
    delivered_text = answer.text if progress is None else progress.edits[-1]["text"]
    assert delivered_text == NOVA_COMPANION_RECALL_ACTION_SUPPRESSED_TEXT
    assert sentinel not in delivered_text
    if progress is None:
        assert answer.markup_edits == []
    async with db.sessions() as session:
        contents = tuple(await session.scalars(select(ConversationMessage.content)))
    ...
    assert bot.nova_companion_captures._screens == {}
    assert bot.nova_companion_captures._capabilities == {}
    assert bot.nova_companion_reminders._screens == {}
    assert bot.nova_companion_reminders._capabilities == {}
    assert len(fake_ai.companion_calls) == 1
...
@pytest.mark.parametrize("source", ["text", "voice"])
async def test_unavailable_recall_topic_is_bounded_for_full_telegram_input(
    db,
    fake_ai,
    caplog,
    source,
    ...
    sentinel = "PRIVATE_RECALL_TOPIC_BEYOND_BOUND"
    ...
    answer = sent_answer(incoming) if progress is None else progress
    delivered_text = answer.text if progress is None else progress.edits[-1]["text"]
    assert delivered_text == NOVA_COMPANION_RECALL_UNAVAILABLE_TEXT
    assert len(delivered_text.encode("utf-16-le")) // 2 <= 4096
    assert sentinel not in delivered_text
    assert sentinel not in caplog.text
    assert fake_ai.companion_calls == []
    assert await companion_counts(db) == (0, 0, 0)
...
def test_recall_topic_rejects_control_characters_without_reflection():
    sentinel = "PRIVATE_CONTROL_TOPIC"
    phrase = f"напомни про наш разговор о безопасной теме\u0007{sentinel}"

    assert NovaCompanionHandlers._nova_companion_recall_topic(phrase) is None
    assert (
        NovaCompanionHandlers._nova_companion_recall_unavailable_answer(phrase)
        == NOVA_COMPANION_RECALL_UNAVAILABLE_TEXT
    )
...
def test_recall_topic_has_independent_character_and_utf8_bounds(topic, expected):
    phrase = f"напомни про наш разговор о {topic}"

    assert NovaCompanionHandlers._nova_companion_recall_topic(phrase) == expected


async def test_oversize_specific_recall_topic_cannot_use_unrelated_generic_anchor(
    db,
    fake_ai,
):
    bot, user = await ready_companion_bot(db, fake_ai)
    ...
    await bot.conversation.append(
        ...
        user.telegram_id,
        role="assistant",
        content="Выбрали зелёный чай.",
        source="text",
        intent="companion_answer",
    )
    sentinel = "PRIVATE_OVERSIZE_SPECIFIC_TOPIC"
    phrase = f"напомни про наш разговор о {'я' * 161}{sentinel}"
    incoming = CompanionMessage(phrase, chat_id=user.telegram_id)

    await deliver(bot, user, incoming)

    assert sent_answer(incoming).text == NOVA_COMPANION_RECALL_UNAVAILABLE_TEXT
    assert sentinel not in sent_answer(incoming).text
    assert fake_ai.companion_calls == []
    assert await companion_counts(db) == (2, 0, 0)
    async with db.sessions() as session:
        assert await session.scalar(select(func.count(TaskReminder.id))) == 0
...
async def test_specific_conversation_recall_does_not_infer_details_from_unrelated_recent_turns(
    db,
    fake_ai,
):
    bot, user = await ready_companion_bot(db, fake_ai)
    await bot.conversation.append(
        ...
        user.telegram_id,
        role="user",
        content="Сегодня выбирали продукты на ужин.",
        source="text",
        intent="companion_user",
    )
    await bot.conversation.append(
        ...
        user.telegram_id,
        role="assistant",
        content="Остановились на простом супе.",
        source="text",
        intent="companion_answer",
    )
    phrase = "напомни про наш с тобой разговор о ментальных тренировках, что именно мы обсуждали?"
    incoming = CompanionMessage(phrase, chat_id=user.telegram_id)

    await deliver(bot, user, incoming)

    assert "ментальных тренировках" in sent_answer(incoming).text
    assert "деталей" in sent_answer(incoming).text.casefold()
    assert fake_ai.companion_calls == []
    assert await companion_counts(db) == (2, 0, 0)


@pytest.mark.parametrize("source", ["text", "voice"])
async def test_bare_remind_please_without_anchor_asks_one_local_semantic_question(
    db,
    fake_ai,
    source,
):
    bot, user = await ready_companion_bot(db, fake_ai)
    incoming = CompanionMessage("напомни плиз", chat_id=user.telegram_id)
    progress = (
        CompanionMessage("Распознаю", chat_id=user.telegram_id) if source == "voice" else None
    )
...
@pytest.mark.parametrize("source", ["text", "voice"])
async def test_bare_remind_please_with_recent_conversation_recalls_without_action_proposals(
    db,
    fake_ai,
    source,
):
    ...
    await bot.conversation.append(
        ...
        user.telegram_id,
        role="user",
        content="Мы обсуждали утренние ментальные тренировки по десять минут.",
        source="text",
        intent="companion_user",
    )
    await bot.conversation.append(
        ...
        user.telegram_id,
        role="assistant",
        content="Да, это был небольшой ежедневный эксперимент.",
        source="text",
        intent="companion_answer",
    )
    fake_ai.companion_result = NovaCompanionResponse(
    ...
    explicit_recall = CompanionMessage(
        "напомни, о чём мы говорили",
        chat_id=user.telegram_id,
    )
    await deliver(bot, user, explicit_recall)
    fake_ai.companion_result = NovaCompanionResponse(
        answer="Мы обсуждали десятиминутные утренние ментальные тренировки.",
        capture=NovaCompanionCapture(kind="note", title="ментальные тренировки"),
    )
    incoming = CompanionMessage("напомни плиз", chat_id=user.telegram_id)
    progress = (
        CompanionMessage("Распознаю", chat_id=user.telegram_id) if source == "voice" else None
    ...
    answer = sent_answer(incoming) if progress is None else progress
    delivered_text = answer.text if progress is None else progress.edits[-1]["text"]
    assert delivered_text == "Мы обсуждали десятиминутные утренние ментальные тренировки."
    assert len(fake_ai.companion_calls) == 2
    assert await companion_counts(db) == (6, 0, 0)
    async with db.sessions() as session:
        assert await session.scalar(select(func.count(TaskReminder.id))) == 0
    assert (
        await bot.reminder_sessions.current(
...
@pytest.mark.parametrize("source", ["text", "voice"])
async def test_bare_remind_please_rejects_unrelated_immediate_conversation_anchor(
    db,
    fake_ai,
    source,
):
    ...
    answer = sent_answer(incoming).text if progress is None else progress.edits[-1]["text"]
    assert answer == NOVA_COMPANION_RECALL_CLARIFICATION_TEXT
    assert fake_ai.companion_calls == []
    assert await companion_counts(db) == (2, 0, 0)


async def test_bare_remind_please_rejects_replaced_old_recall_anchor(db, fake_ai):
    bot, user = await ready_companion_bot(db, fake_ai)
    for role, content in (
        ("user", "напомни, о чём мы говорили"),
        ("assistant", "Мы обсуждали ментальные тренировки."),
        ("user", "Теперь хочу выбрать чай на ужин."),
        ("assistant", "Давай сравним зелёный и чёрный чай."),
    ):
        await bot.conversation.append(
            user.telegram_id,
            user.telegram_id,
            ...
            content=content,
            source="text",
            intent="companion_user" if role == "user" else "companion_answer",
        )
    incoming = CompanionMessage("напомни плиз", chat_id=user.telegram_id)

    await deliver(bot, user, incoming)

    assert sent_answer(incoming).text == NOVA_COMPANION_RECALL_CLARIFICATION_TEXT
    assert fake_ai.companion_calls == []
    assert await companion_counts(db) == (4, 0, 0)
...
async def test_vague_frequency_is_never_accepted_as_a_real_schedule(
    ...
    answer = sent_answer(incoming)
    assert answer.text == NOVA_COMPANION_NOT_EXECUTED_TEXT
    assert answer.markup_edits == []
    assert (
    ...
    async with db.sessions() as session:
        ...
        assert NOVA_COMPANION_NOT_EXECUTED_TEXT in contents
        assert await session.scalar(select(func.count(TaskReminder.id))) == 0
    assert len(fake_ai.companion_calls) == 1
    assert bot._nova_companion_tasks == set()
...
async def test_rejected_reminder_offer_never_delivers_or_persists_dependent_answer(
    ...
    assert "PRIVATE_REJECTED_OFFER" not in answer.text
    async with db.sessions() as session:
        contents = tuple(await session.scalars(select(ConversationMessage.content)))
    ...
    assert len(fake_ai.companion_calls) == 1
    assert "PRIVATE_REJECTED_OFFER" not in caplog.text


@pytest.mark.parametrize(
    ("claim", "expected_answer"),
    [
        (
            "Могу поставить напоминание — нажми кнопку.",
            NOVA_COMPANION_REJECTED_REMINDER_OFFER_TEXT,
        ),
        (
            "Могу поставить напоминание — выбери кнопку.",
            NOVA_COMPANION_REJECTED_REMINDER_OFFER_TEXT,
        ),
        (
            "Могу поставить напоминание, кнопка ниже.",
            NOVA_COMPANION_REJECTED_REMINDER_OFFER_TEXT,
        ),
        (
            "Могу поставить напоминание.",
            NOVA_COMPANION_REJECTED_REMINDER_OFFER_TEXT,
        ),
        ("Создам напоминание.", NOVA_COMPANION_NOT_EXECUTED_TEXT),
        ("Настрою напоминание.", NOVA_COMPANION_REJECTED_REMINDER_OFFER_TEXT),
        ("Готово.", NOVA_COMPANION_REJECTED_REMINDER_OFFER_TEXT),
    ],
)
async def test_malformed_reminder_offer_never_delivers_action_dependent_answer(
    db,
    fake_ai,
    caplog,
    claim,
    ...
    sentinel = "PRIVATE_REJECTED_ACTION_ANSWER"
    raw_answer = f"{claim} {sentinel}"
    ...
    assert sent_answer(incoming).text == expected_answer
    assert raw_answer not in sent_answer(incoming).text
    async with db.sessions() as session:
        contents = tuple(await session.scalars(select(ConversationMessage.content)))
        assert contents == (incoming.text, expected_answer)
        assert await session.scalar(select(func.count(DraftInboxItem.id))) == 0
        assert await session.scalar(select(func.count(InboxItem.id))) == 0
        assert await session.scalar(select(func.count(TaskReminder.id))) == 0
    assert (
...
@pytest.mark.parametrize(
    ("scenario", "incoming_text", "title", "schedule", "evidence", "history"),
    [
        (
            "forged",
            "А вдруг забуду?",
        ...
        (
            ...
            (
                ("user", "А вдруг забуду?"),
                ("assistant", "Я поставила напоминание про врача"),
            ),
        ),
        (
            "ambiguous",
            ...
            "стрижка",
            "завтра в 19:00",
            "Стрижка завтра в 19:00",
            (
                ("user", "Стрижка завтра в 19:00"),
                ("assistant", "Что ещё важно?"),
                ("user", "Позвонить врачу послезавтра в 10:00"),
            ),
        ),
        (
            "invalid_schedule",
...
async def test_rejected_grounding_cases_replace_raw_answer_and_create_nothing(
    db,
    fake_ai,
    caplog,
    scenario,
    ...
    sentinel = f"PRIVATE_REJECTED_{scenario.upper()}"
    raw_answer = f"Могу поставить напоминание — кнопка ниже. {sentinel}"
    fake_ai.companion_provider_raw_result = {
        "answer": raw_answer,
        "reminder_offer": {
            "title": title,
    ...
    incoming = CompanionMessage(incoming_text, chat_id=user.telegram_id)

    with caplog.at_level(logging.WARNING):
        await deliver(bot, user, incoming)

    assert sent_answer(incoming).text == NOVA_COMPANION_REJECTED_REMINDER_OFFER_TEXT
    async with db.sessions() as session:
        contents = tuple(await session.scalars(select(ConversationMessage.content)))
        assert raw_answer not in contents
        assert NOVA_COMPANION_REJECTED_REMINDER_OFFER_TEXT in contents
...
async def test_conflicting_action_proposals_replace_dependent_answer_and_create_nothing(
    db,
    fake_ai,
):
    text = "Я думаю о прогулке и о визите к врачу завтра в 19:00."
    raw_answer = "Выбери кнопку ниже. PRIVATE_CONFLICTING_ACTIONS"
    fake_ai.companion_provider_raw_result = {
        "answer": raw_answer,
        "capture": {
            "kind": "idea",
...
async def test_prior_grounded_subject_and_current_vague_recurrence_start_guided_flow(
    ...
    bot, user = await ready_companion_bot(
        db,
        fake_ai,
        telegram_user_id=79_212 + (source == "voice"),
    )
    context = companion_context()
    fake_ai.companion_result = NovaCompanionResponse(
...
@pytest.mark.parametrize("source", ["text", "voice"])
async def test_immediate_recall_question_answer_pair_grounds_ambiguous_recall(
    db,
    fake_ai,
    source,
):
    bot, user = await ready_companion_bot(
        db,
        fake_ai,
        telegram_user_id=79_220 + (source == "voice"),
    )
    context = companion_context()
    fake_ai.companion_result = NovaCompanionResponse(
        answer="Это может вернуть чтению спокойное место в твоём дне."
    )
    await deliver(
        bot,
        user,
    ...
    fake_ai.companion_result = NovaCompanionResponse(
        answer="Мы говорили о возвращении к художественной литературе."
    )
    recall = CompanionMessage("О чём мы сейчас говорили?", chat_id=user.telegram_id)
    await deliver(bot, user, recall, context=context)

    fake_ai.companion_result = NovaCompanionResponse(
        answer="Мы говорили о том, как снова начать читать художественную литературу.",
        reminder_offer=NovaCompanionReminderOffer(
            title="читать художественную литературу",
            evidence="О чём мы сейчас говорили?",
        ),
    )
    incoming = CompanionMessage("Напомни плиз", chat_id=user.telegram_id)
    progress = (
    ...
    answer = sent_answer(incoming).text if progress is None else progress.edits[-1]["text"]
    assert answer == ("Мы говорили о том, как снова начать читать художественную литературу.")
    assert answer != NOVA_COMPANION_RECALL_CLARIFICATION_TEXT
    ...
    async with db.sessions() as session:
        ...
        assert await session.scalar(select(func.count(TaskReminder.id))) == 0
        contents = tuple(await session.scalars(select(ConversationMessage.content)))
    assert contents[-2:] == ("Напомни плиз", answer)
    assert bot._nova_companion_tasks == set()
...
@pytest.mark.parametrize(
    "phrase",
    [
        "мне тут Марина сказала что я Оля",
        "Марина сказала, что я Оля",
        "Мне сказали, что меня зовут Оля",
        "Марина сказала: “Меня зовут Оля”. Как меня зовут?",
    ],
)
async def test_reported_third_party_identity_cannot_override_authoritative_profile(
    db,
    fake_ai,
    source,
    phrase,
    ...
    answer = sent_answer(incoming).text if progress is None else progress.edits[-1]["text"]
    if "Как меня зовут" in phrase:
        assert answer == "Да, тебя зовут Назар."
    else:
        assert "Подтверждённое имя в профиле — Назар" in answer
    assert "Оля" not in answer
    assert fake_ai.companion_calls == []
    async with db.sessions() as session:
        actor = await session.get(User, user.id)
...
@pytest.mark.parametrize(
    ("phrase", "expected"),
    [
        ("Марина сказала, что я устал", False),
        ("Друг сказал, что я молодец", False),
        ("Марина говорит, что я слишком много работаю", False),
        ("Мне сказали, что я хорошо выступил", False),
        ("Марина сказала, что я Оля", True),
        ("мне тут Марина сказала что я Оля", True),
        ("Мне сказали, что меня зовут Оля", True),
        ("Марина сказала: «Меня зовут Оля». Как меня зовут?", True),
    ],
)
def test_reported_identity_helper_distinguishes_names_from_predicates(phrase, expected):
    assert _is_reported_identity_claim(phrase) is expected
...
async def test_reported_predicates_remain_companion_conversation_after_restart_and_overflow(
    db,
    fake_ai,
    source,
    phrase,
):
    telegram_id = 79_260 + 10 * (source == "voice") + len(fake_ai.companion_calls)
    bot, user = await ready_companion_bot(
        db,
        fake_ai,
    ...
    for index in range(bot.settings.conversation_context_messages + 3):
        await bot.conversation.append(
            ...
            user.telegram_id,
            role="user" if index % 2 == 0 else "assistant",
            content=f"безопасная реплика переполнения {index}",
            source="text",
            intent="companion_user" if index % 2 == 0 else "companion_answer",
        )
    restarted = FutureSelfBot(bot.settings, db, fake_ai, NoopTranscription())
    user = await restarted._user(user.telegram_id)
    safe_answer = "Это звучит как пересказ чужой оценки, а не как новое имя."
    fake_ai.companion_provider_result = NovaCompanionProviderResponse(answer=safe_answer)
    async with db.sessions() as session:
        before_messages = int(await session.scalar(select(func.count(ConversationMessage.id))) or 0)
        before_memories = int(await session.scalar(select(func.count(NovaObservedMemory.id))) or 0)
    incoming = CompanionMessage(phrase, chat_id=user.telegram_id)
    progress = (
        CompanionMessage("Распознаю…", chat_id=user.telegram_id) if source == "voice" else None
    )
    ...
    assert "Подтверждённое имя" not in answer
    assert len(fake_ai.companion_calls) == 1
    async with db.sessions() as session:
        actor = await session.get(User, user.id)
        contents = tuple(
            await session.scalars(
                select(ConversationMessage.content).order_by(ConversationMessage.id.desc()).limit(2)
            )
        )
        after_messages = int(await session.scalar(select(func.count(ConversationMessage.id))) or 0)
        after_memories = int(await session.scalar(select(func.count(NovaObservedMemory.id))) or 0)
    assert actor is not None and actor.display_name == "Назар"
    assert contents == (safe_answer, phrase)
    assert after_messages == before_messages
    assert after_memories == before_memories == 0
    assert restarted._nova_companion_tasks == set()


@pytest.mark.parametrize("source", ["text", "voice"])
@pytest.mark.parametrize("provider_actions", ["reminder", "capture", "conflict", "state"])
@pytest.mark.parametrize(
    ("command", "semantic_key", "normalized_value", "acknowledgement"),
    [
        (
            "Отвечай мне коротко",
            "response_length",
...
async def test_server_grounded_setting_suppresses_every_provider_action_and_state(
    db,
    fake_ai,
    caplog,
    source,
    ...
    bot, user = await ready_companion_bot(
        db,
        fake_ai,
        telegram_user_id=79_300 + (source == "voice"),
        enable_brain=True,
    )
    sentinel = f"PRIVATE_SETTING_{semantic_key.upper()}_{provider_actions.upper()}"
    provider_title = command if provider_actions == "reminder" else f"{sentinel}_TITLE"
    raw: dict[str, object] = {
        "answer": f"Нажми кнопку ниже, чтобы применить действие. {sentinel}",
    ...
    if provider_actions in {"capture", "conflict"}:
        raw["capture"] = {
            "kind": "note",
            "title": provider_title,
            "evidence": command,
        }
    if provider_actions in {"reminder", "conflict"}:
        raw["reminder_offer"] = {
            "title": provider_title,
            "evidence": command,
        }
    ...
    delivered = sent_answer(incoming).text if progress is None else progress.edits[-1]["text"]
    assert delivered == acknowledgement
    ...
    assert contents == (command, acknowledgement)
    assert len(memories) == 1
    assert memories[0].status == "active"
    assert memories[0].semantic_key == semantic_key
    assert memories[0].normalized_value == normalized_value
    assert sentinel not in "\n".join(contents)
    assert sentinel not in caplog.text
    assert "invalid_dialogue_state" not in caplog.text
    assert "invalid_memory_candidate" not in caplog.text
    if provider_actions == "reminder":
        assert "invalid_reminder_offer" not in caplog.text
    assert bot._nova_companion_tasks == set()


async def test_server_grounded_setting_post_provider_access_change_is_noop(
    db,
    fake_ai,
    monkeypatch,
):
    bot, user = await ready_companion_bot(db, fake_ai, enable_brain=True)
    command = "Говори со мной прямо"
    fake_ai.companion_provider_result = NovaCompanionProviderResponse(
        answer="PRIVATE_STALE_SETTING_ANSWER",
        capture=NovaCompanionProviderCapture(
            kind="note",
            title="PRIVATE_STALE_SETTING_CAPTURE",
            evidence=command,
        ),
    )
    outcomes: AsyncIterator[str] = _outcomes(("ready", "access_changed"))
...
async def test_server_grounded_setting_cancellation_before_provider_result_is_noop(
    db,
    fake_ai,
):
    bot, user = await ready_companion_bot(db, fake_ai, enable_brain=True)
    command = "Отвечай мне коротко"
    fake_ai.companion_provider_result = NovaCompanionProviderResponse(
        answer="PRIVATE_CANCELLED_SETTING_ANSWER"
    )
    fake_ai.companion_release.clear()
    incoming = CompanionMessage(command, chat_id=user.telegram_id)
    snapshot = await bot.conversation.get(user.telegram_id, user.telegram_id)
...
async def test_concurrent_memory_replacement_invalidates_server_grounded_setting(
    db,
    fake_ai,
):
    bot, user = await ready_companion_bot(db, fake_ai, enable_brain=True)
    command = "Говори со мной прямо"
    fake_ai.companion_provider_result = NovaCompanionProviderResponse(
        answer="PRIVATE_CONCURRENT_SETTING_ANSWER"
    )
    fake_ai.companion_release.clear()
    incoming = CompanionMessage(command, chat_id=user.telegram_id)
    snapshot = await bot.conversation.get(user.telegram_id, user.telegram_id)
    ...
    async with db.sessions() as session:
        ...
        assert await session.scalar(select(func.count(NovaDialogueState.id))) == 0
    assert len(memories) == 1
    assert memories[0].normalized_value == "tone=calm"
    assert bot.nova_companion_captures._capabilities == {}
    assert bot.nova_companion_reminders._capabilities == {}
    assert bot._nova_companion_tasks == set()
...
@pytest.mark.parametrize("source", ["text", "voice"])
async def test_server_grounded_settings_supersede_history_and_weak_continuation_is_local(
    db,
    fake_ai,
    source,
):
    bot, user = await ready_companion_bot(
        db,
        fake_ai,
        telegram_user_id=79_240 + (source == "voice"),
        enable_brain=True,
    )
    async with db.session() as session:
        session.add_all(
    ...
    fake_ai.companion_result = NovaCompanionResponse(answer="Служебный ответ provider.")
    context = companion_context()
    for index, command in enumerate(
        (
            "Нова, отвечай мне подробно, пожалуйста",
            "Говори со мной прямо",
            "Обращайся ко мне в мужском роде",
            "Я предпочитаю краткие напоминания",
            "Я предпочитаю краткие напоминания",
        ...
        fake_ai.companion_error = (
            RuntimeError("PRIVATE_SETTING_PROVIDER_FAILURE") if index == 0 else None
        )
        await deliver(
            bot,
            user,
            CompanionMessage(command, chat_id=user.telegram_id),
            context=context,
        )
    ...
    answer = sent_answer(query).text if progress is None else progress.edits[-1]["text"]
    assert answer == (
        "Ты попросил отвечать подробно, говорить прямо, напоминать кратко и "
        "обращаться к тебе в мужском роде"
    )
    continuation = CompanionMessage("а ещё", chat_id=user.telegram_id)
    await deliver(bot, user, continuation, context=context)
    assert sent_answer(continuation).text == (
        "Это все активные настройки общения, которые у меня сейчас сохранены."
    )
    assert len(fake_ai.companion_calls) == provider_calls
    async with db.sessions() as session:
        rows = tuple(
            await session.execute(
                select(
                    NovaObservedMemory.semantic_key,
                    NovaObservedMemory.normalized_value,
                    NovaObservedMemory.status,
                ).order_by(NovaObservedMemory.id)
            )
        )
    active = {(key, value) for key, value, status in rows if status == "active"}
    assert active == {
        ("response_length", "response_length=detailed"),
        ("tone", "tone=direct"),
        ("identity", "identity:grammatical_address=masculine"),
        ("reminder_style", "reminder_style=brief"),
    }
    assert (
        sum(
            1
            for key, value, _status in rows
            if key == "reminder_style" and value == "reminder_style=brief"
        )
        == 1
    )
    for index in range(bot.settings.conversation_context_messages + 3):
    ...
    restarted = FutureSelfBot(bot.settings, db, fake_ai, NoopTranscription())
    restarted_user = await restarted._user(user.telegram_id)
    stale_continuation = CompanionMessage("а ещё", chat_id=user.telegram_id)
    await deliver(restarted, restarted_user, stale_continuation, context=companion_context())
    assert sent_answer(stale_continuation).text != (
        "Это все активные настройки общения, которые у меня сейчас сохранены."
    )
    provider_calls_after_unanchored_continuation = len(fake_ai.companion_calls)
    restarted_query = CompanionMessage(
        "Какие настройки общения ты обо мне помнишь?",
        chat_id=user.telegram_id,
    )
    await deliver(restarted, restarted_user, restarted_query, context=companion_context())
    assert sent_answer(restarted_query).text == answer
    assert len(fake_ai.companion_calls) == provider_calls_after_unanchored_continuation
    assert bot._nova_companion_tasks == set()
    assert restarted._nova_companion_tasks == set()
